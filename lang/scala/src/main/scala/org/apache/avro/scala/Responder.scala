...
class Responder[P <: HList](handler: ProtocolHandler[P], local: Protocol)(implicit rpc: Rpc[P], encoder: Encoder, decoder: Decoder) extends GenericResponder(local.avroProtocol) {
  @throws[Exception]
  override def respond(avroMessage: AvroProtocol#Message, genericRequest: AnyRef): AnyRef = {
    val message = Message.fromAvro(avroMessage)
    ...
    val parameters = message.parameters.map { param =>
      ...
      output.toByteArray.toString
    }
    rpc.handlers(message.name)(handler.impl$, parameters, encoder, decoder)
  }
}
